...
class UiDesktop:
    ...
    def __init__(self, ws: UiConsoleBroker):
        ...
        root.geometry("1000x500")  # Aumenta o tamanho da tela
        
        self.root=root

        # Create a Canvas to hold the frames
        ...
        # Create a frame inside the canvas to hold the channel controls
        ...
        for id, input in enumerate(ws.inputs): 
            ...
            name_var = tk.StringVar(name=f"input-{id}-name", value=ws.inputs[id].name_subject.cached)
            ...
            ws.inputs[id].name_subject.add_listener(lambda value, var=name_var: var.set(value))
            
            forceunmute_var = tk.BooleanVar(name=f"input-{id}-mute", value=ws.inputs[id].forceunmute_subject.cached)
            tk.Checkbutton(
                frame,
            ...
            ws.inputs[id].forceunmute_subject.add_listener(lambda value, var=forceunmute_var: var.set(value))
            
            stereo_index_var = tk.IntVar(name=f"input-{id}-stereo_index", value=ws.inputs[id].stereo_index_subject.cached)
            ...
            ws.inputs[id].stereo_index_subject.add_listener(lambda value, var=stereo_index_var: var.set(value))

            volume_var = tk.DoubleVar(name=f"input-{id}-volume", value=ws.inputs[id].mix_subject.cached)
            tk.Scale(
                frame,
            ...
            ws.inputs[id].mix_subject.add_listener(lambda value, var=volume_var: var.set(value)) # Removed * 255
